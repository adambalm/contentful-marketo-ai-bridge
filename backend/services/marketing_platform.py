...
class AsyncMarketingAdapter:
    ...
    def __init__(self, service):
        ...
        self._is_async = hasattr(
            service, "add_to_list"
        ) and asyncio.iscoroutinefunction(service.add_to_list)
    ...
    async def add_to_list(
        ...
        if self._is_async:
            ...
            return await loop.run_in_executor(
                None, self.service.add_to_list, list_id, leads
            )
    ...
    async def test_connection(self) -> dict[str, Any]:
        ...
        return {
            "success": True,
            "platform": "unknown",
            "message": "No test method available",
        }
            ...
            "result": [
                {
                    "id": i + 1,
                    "status": "added",
                    "email": lead.get("email", f"demo-{i}@example.com"),
                }
...
class MarketingPlatformFactory:
    ...
    def create_service() -> AsyncMarketingAdapter:
        ...
        if platform == "marketo":
            ...
            print(
                f"Warning: Unknown MARKETING_PLATFORM '{platform}', defaulting to mock service"
            )
            service = MockMarketingService()
